...
- **Change ativo**: nenhum — os últimos changes foram arquivados em
  ...
  `openspec/archive/fix-openai-strict-oneof-normalization/`,
  `openspec/archive/fix-doctor-decision-scroll-to-form-errors/` e
  `openspec/archive/fix-doctor-decision-error-banner/`.
- **Changes concluídos**:
...
- **Testes**: 3139 passando (gate final do bugfix `fix-doctor-decision-error-banner`;
  inclui testes de contrato do vínculo strict V2, da normalização `oneOf`→`anyOf`, do
  banner de erro na decisão médica e os testes de contrato de documentação
