...
## Change Ativo — Catálogo ampliado de procedimentos endoscópicos

- **Change:** `openspec/changes/support-expanded-endoscopy-procedure-catalog/` — proposal, design (D1–D15), 9 specs e 10 slices; **implementação CONCLUÍDA na branch em 2026-09-25** (10 slices verticais aceitos com reviewer independente; gate final verde 4445 testes), aguardando smoke em ambiente real (R4/R5 do Slice 010) e aprovação humana para arquivar/deployar.
- **Branch:** `feature/support-expanded-endoscopy-procedure-catalog` (BASE_REF `38cbd0f`; commits `cc23754`…`1303723`); baseline global registrado em `.pi/reports/baseline-support-expanded-endoscopy-procedure-catalog-2026-09-25.md` (3835 testes) → gate final 4445. Runbook de cutover: `docs/deploy/expanded-endoscopy-procedure-catalog-cutover.md`; precheck de downgrade evoluiu para a fronteira `first_4_0_write` (3.0/eco-CPRE = baseline informacional).
...
- **Baseline atual do código:** writer 3.0, catálogo `eda|colonoscopy|echoendoscopy|cpre` (quatro tipos).
- **Objetivo 4.0/dez tipos:** `eda`, `eda_gastrostomy`, `eda_capsule`, `eda_dilation`, `colonoscopy`, `rectosigmoidoscopy`, `rectosigmoidoscopy_dilation`, `rectosigmoidoscopy_argon`, `echoendoscopy`, `cpre`. Pacotes com `+` são identidades indivisíveis (uma row, uma decisão, uma recomendação); EDA + Colonoscopia permanece duas rows com decisão independente e agendamento casado. Perfis reutilizados por família (variações de EDA → profile EDA; Retossigmoidoscopias → profile Colonoscopia) sem equivalência de histórico/filtro/analytics (código exato). Local anatômico informativo em EDA + Dilatação; painel consultivo de possível infecção sistêmica em EDA + GTT (resultados presentes — inclusive normais — visíveis; alerta apenas com preocupação explicitamente documentada; nunca altera policy/recomendação/decisão/FSM).
- **UI:** seletores de procedimento viram combobox pesquisável (fallback SSR `<select>`, validação backend por código exato); badges CSS por família.
- **Operação:** sem flags novas, sem backfill, sem rollout gradual — cutover único de web/workers/prompts 4.0 (drenar jobs 3.0); após o primeiro write 4.0, rollback suportado é fix-forward.
- **Regra de reentrada:** implementar um slice vertical por vez conforme `tasks.md`; nenhuma implantação de estados intermediários da branch.
...
No baseline atual, o sistema faz **triagem automatizada para EDA e Colonoscopia** (Endoscopia Digestiva Alta e Baixa). O change ativo (`support-expanded-endoscopy-procedure-catalog`) amplia o catálogo para dez identidades atômicas — variações indivisíveis de EDA (GTT, cápsula endoscópica, dilatação) e a família Retossigmoidoscopia — sob contrato gravável 4.0, além de Ecoendoscopia e CPRE independentes já entregues. Operadores NIR enviam PDFs de relatorios medicos, o
...
- **CaseFollowUp / ProcedureFollowUp**: versões append-only do desfecho pós-procedimento. Novas rows usam a taxonomia oficial fechada; rows legadas preservam códigos/detalhes originais e são projetadas apenas na leitura analítica. Cada gravação gera `FOLLOWUP_RECORDED`/`FOLLOWUP_UPDATED`; Histórico e exportação nunca reescrevem rows ou eventos.
...
- **Pós-procedimento — taxonomia oficial e legado:** novas gravações aceitam exatamente as 23 causas da ficha oficial + `other`; `other` exige texto e as demais causas o rejeitam; qualquer `resource_shortage_detail` é inválido para write novo. O storage continua reconhecendo `absenteeism`/`resource_shortage` para leitura. `project_non_performance_reason` mapeia somente `absenteeism → patient_no_show` e os três pares confirmados de falta de recursos; causa atual só passa com detalhe vazio. Qualquer outro par retorna `legacy_unmapped`, fica fora do filtro oficial e faz `unmapped_legacy_follow_up_ids()` bloquear rollout. `CaseFollowUp`, `ProcedureFollowUp` e `CaseEvent` históricos permanecem append-only.
...
- **Fase atual:** change CRÍTICO `support-expanded-endoscopy-procedure-catalog` **implementado na branch** `feature/support-expanded-endoscopy-procedure-catalog` (BASE_REF `38cbd0f`; gate final verde: 4445 testes, ruff/mypy clean, `openspec validate --strict` ok) — pendências: smoke funcional/acessível em ambiente real (sem browser neste host) e aprovação humana antes de arquivar/deployar; produção segue em 3.0/quatro tipos até o cutover 4.0/dez tipos pelo runbook.
...
- **Changes concluídos**:
  - `openspec/archive/align-followup-causes-with-suspension-form/` (2 slices — catálogo oficial com 23 causas + Outras causas, select compacto, rejeição de writes legados/submotivo, migration metadata-only e projeção histórica dos quatro mapeamentos confirmados em tabela/cards/filtro/CSV; fallback/preflight fail-closed `legacy_unmapped`; manual atualizado; sem backfill ou reescrita de eventos).
  ...
  - `openspec/archive/fix-llm2-reconciled-procedure-set/` (1 slice — hotfix do pipeline clínico: o LLM2 agora recebe cópia efêmera do artefato LLM1 filtrada pelo conjunto reconciliado, sem alterar `Case.structured_data`; o prompt declara lista canônica fechada; igualdade exata continua fail-closed; mismatch schema-válido possui erro tipado e um retry one-shot independente do retry pt-BR, limitado a três chamadas físicas; EDA-only, Colonoscopia-only e combinado cobertos; sem migration, FSM, schema, detector ou recuperação automática de `FAILED`).
  ...
  - `openspec/archive/fix-openai-strict-oneof-normalization/` (hotfix de produção pós-`v0.3.0-rc.2` — a API recusava o schema V2 do LLM1 com `400 invalid_json_schema` porque a união discriminada de `requested_procedures` serializa como `oneOf` + `discriminator`, não suportados pelo modo strict. O normalizador `_normalize_openai_strict_schema` passou a reescrever `oneOf`→`anyOf` e remover `discriminator` apenas na cópia enviada à API; modelos Pydantic e validação local inalterados. Spec `procedure-neutral-analysis` ganhou requisito de compatibilidade de uniões. Sem migration).
  - `openspec/archive/fix-doctor-decision-scroll-to-form-errors/` (bugfix QUICK de UX — após erro de validação no submit da decisão médica, a página re-renderizada voltava ao topo e os erros ficavam fora da viewport. O formulário recebe o marcador SSR `data-scroll-to-errors` somente quando há erros, e `decision.js` rola até o primeiro campo `.is-invalid`. Spec `per-procedure-medical-decision` ganhou requisito de reposicionamento. Sem migration). **Superado**: o scroll automático não funcionou em teste manual de produção; substituído pelo `fix-doctor-decision-error-banner`.
  - `openspec/archive/fix-doctor-decision-error-banner/` (bugfix QUICK de UX — substitui o scroll automático por um banner `alert-danger` no topo da tela de decisão médica quando há erro de validação, com contagem de campos e botão de âncora nativa `#doctor-decision-form`; remove o marcador `data-scroll-to-errors` e o bloco de scroll JS. Requisito de reposicionamento da spec `per-procedure-medical-decision` modificado. Sem migration).
