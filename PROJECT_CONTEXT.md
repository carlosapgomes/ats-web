...
- **Change ativo**: nenhum — os últimos changes foram arquivados em
  ...
  `openspec/archive/fix-openai-strict-oneof-normalization/` e
  `openspec/archive/fix-doctor-decision-scroll-to-form-errors/`.
- **Changes concluídos**:
...
- **Testes**: 3139 passando (gate final do bugfix `fix-doctor-decision-scroll-to-form-errors`;
  inclui testes de contrato do vínculo strict V2, da normalização `oneOf`→`anyOf`, do
  marcador de scroll no formulário de decisão médica e os testes de contrato de documentação
