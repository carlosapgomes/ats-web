...
Define a política de visibilidade da lista de notificações in-app do usuário: apenas não lidas e leituras dentro da janela de retenção (medida por `read_at`) são exibidas; ocultação é somente de leitura, sem deleção. Promovida a partir do change `notifications-read-visibility`.
## Requirements
