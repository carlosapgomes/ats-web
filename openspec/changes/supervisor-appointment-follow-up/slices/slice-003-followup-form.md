...
- P2s diferidos: (a) `_followup_form_context` duplica lógica de ramo (centralizar classificador); ~~(b) docstring de view ainda diz que cards não linkam~~ e ~~(c) `_enrich_followup_item` (listagem) mantém `is_immediate` só pelo fluxo (híbridos mostram label do fluxo)~~ **fechados pelo mini-change `followup-hybrid-card-precedence`**; (d) truncamento `agency_record_number`; (e) pill sem estado ativo; (f) `EVENT_DOT_CSS` sem entrada p/ FOLLOWUP_* (fallback "system").
