...
- **R3:** mismatch especializado oferece correção somente quando flag ativa e antes de `WAIT_DOCTOR`. **Dívida herdada do Slice 002 (P2 da revisão):** `correct_case_exam_type`/`validate_exam_type` em `apps/intake/services.py` aceitam `echoendoscopy` sem consultar `is_echoendoscopy_intake_enabled`; inserir o gate (reutilizar `ensure_exam_type_allowed` ou equivalente) para que correção com flag off seja rejeitada, cobrindo também CPRE quando o Slice 004 o liberar. **Dívida herdada do Slice 004 (P2 da revisão, pré-existente desde o Slice 002, afeta Eco e CPRE):** quando a reconciliação retorna `nir_review` com conjunto detectado fora de `ALLOWED_PROCEDURE_SETS` (ex.: declarado `eda` + `Solicito EDA. Solicito CPRE.` independentes), `set_detected_procedures` → `normalize_detected_set` levanta `ValueError` e o caso termina em `PIPELINE_FAILED` em vez do estado de revisão NIR desenhado (D2: conjuntos incompatíveis ao NIR com motivo explícito). Forma mínima derivada da análise da revisão: projetar detecção somente quando `reconciliation.action != "nir_review"` (ou gate equivalente preservando a projeção para singletons válidos de mismatch), em `apps/pipeline/orchestrator.py`/`apps/cases/procedures.py`. Critério de aceite: testes end-to-end para Eco e CPRE afirmando `EDA_SCOPE_GATED_MANUAL_REVIEW` + `scope_gate_bypass` + motivo `unsupported_procedure_combination` com `len(client.calls) == 1`.
