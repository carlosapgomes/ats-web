...
- [ ] Slice 001 — Reescrever `oneOf`→`anyOf` e remover `discriminator` no normalizador com teste de contrato (`slices/slice-001-oneof-anyof-normalization.md`)

## Definition of Done do change

- [ ] `_normalize_openai_strict_schema` converte `oneOf` em `anyOf` em qualquer nível.
- [ ] `_normalize_openai_strict_schema` remove `discriminator` em qualquer nível.
- [ ] Schema normalizado de `Llm1ResponseV2` não contém `oneOf` nem `discriminator` e mantém a união via `anyOf`.
- [ ] Modelos Pydantic e validação local (`model_validate`) permanecem inalterados.
