...
- [x] 2.3 Implementar o Slice 004 (`slices/slice-004-gastrostomy-infection-review.md`) e provar EDA + GTT do intake ao médico com resultados normais visíveis, alerta explicitamente sustentado e invariância de policy/recomendação/FSM.
...
- [x] 5.2 Executar uma única vez o gate final `uv run ruff check . && uv run ruff format --check . && uv run mypy . && uv run pytest`, seguido de `openspec validate support-expanded-endoscopy-procedure-catalog --strict`, `git diff --check` e `git status --short`; registrar resultados no relatório final. *(2026-09-25, no commit final do código: ruff `All checks passed!`; format `289 files already formatted`; mypy `no issues in 319 files`; pytest **4445 passed** (143s); `openspec validate --strict` valid; `git diff --check` clean; status = somente artefatos de planejamento aprovados + relatórios não rastreados. Neste host o pytest exige `POSTGRES_TEST_HOST_PORT=55433`.)*
- [x] 5.3 Revisar evidências de todos os slices, atualizar somente checkboxes comprovados, gerar relatório final com snippets antes/depois, fazer commit/push rastreáveis, informar `REPORT_PATH` e parar para aprovação humana antes de arquivar. *(Commits `cc23754`…final criados; **push não realizado** por instrução explícita do operador desta sessão — aguardando instrução; arquivamento também aguarda aprovação; smoke R4/R5 pendente em ambiente real.)*
