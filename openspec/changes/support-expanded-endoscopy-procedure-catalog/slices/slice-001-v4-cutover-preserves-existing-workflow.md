...
- `apps/intake/services.py` (derivação de `_DECLARED_SELECTION_VALUES`/conversão a partir do catálogo);
...
- **R2:** a matriz aceita qualquer singleton canônico e somente `{eda, colonoscopy}` como conjunto multi-item; desconhecido e qualquer outro par falham explicitamente; `selection_key`/paired usam igualdade exata; `selection_key` permanece função total e devolve o sentinela `invalid` para conjuntos fora da matriz; `SELECTION_KEYS`/`procedure_types_for_selection()` tornam-se públicas no domínio e `apps/intake/services.py` deriva delas a validação de seleção, mantendo os gates de flag existentes.
- **R3:** schemas LLM1/LLM2 4.0 strict aceitam dez identidades, proíbem duplicata e já definem no schema os detalhes tipados aplicáveis (local de dilatação e coleção de evidências infecciosas — a extração/verificação desses detalhes é ativada nos Slices 003/004); production dispatch e prompts seedados usam 4.0.
...
out_of_scope:
  - expor novas opções no intake (derivar chaves válidas NÃO expõe opções; a UI continua oferecendo somente as atuais)
  - detecção de pacotes/Retossigmoidoscopia
  - painel GTT, detalhe de dilatação, combobox, filas ou analytics
```

Cap esperado: até 19 arquivos, justificado pelo cutover strict coordenado (inclui a derivação das chaves de seleção do intake a partir do catálogo). Se for necessário editar templates, FSM, permissions, scheduler/dashboard ou criar persistência clínica, parar e escalar.

## Matriz requisito → arquivo → teste/check

| Requisito | Arquivo(s) esperado(s) | Teste/check |
| --- | --- | --- |
| R1–R2 | `models.py`, `procedures.py`, `apps/intake/services.py`, migration | `test_expanded_procedure_catalog.py` + regressão `apps/intake/tests/test_exam_type_intake.py` |
...
uv run pytest \
  ...
  apps/intake/tests/test_exam_type_intake.py -q
