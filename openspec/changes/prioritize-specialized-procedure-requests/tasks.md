...
- [ ] 0.3 Antes do primeiro código, registrar `BASE_REF`, confirmar branch/worktree adequadas e executar baseline global uma única vez se não houver CI verde confiável do HEAD.

## 1. Slice vertical

- [ ] 1.1 Implementar o Slice 001 (`slices/slice-001-specialized-precedence-to-doctor.md`) em RED → GREEN → REFACTOR, provar Ecoendoscopia/CPRE até `WAIT_DOCTOR`, auditoria e aviso médico, e obter review independente aceito.

## 2. Gate final e operação

- [ ] 2.1 Executar uma única vez o gate final: `uv run ruff check . && uv run ruff format --check . && uv run mypy . && uv run pytest`.
- [ ] 2.2 Executar `openspec validate prioritize-specialized-procedure-requests --strict`, `git diff --check` e inspeção final de escopo.
