...
A superfície Follow-up SHALL apresentar sub-abas "Registrar" (fluxo de registro existente em `/dashboard/follow-ups/`, comportamento inalterado) e "Histórico & Exportação" (`/dashboard/follow-ups/history/`), com estado ativo visível conforme a página corrente e acesso restrito aos papéis `manager` e `admin`.

#### Scenario: Sub-abas visíveis e ativas

- **GIVEN** um usuário com papel `manager` autenticado
