...
Cutover 4.0 (design D5/D14 / ADR-0010): o seed é canônico para os QUATRO prompts
neutros ``exam_llm{1,2}_{system,user}``. Ao rodar, garante exatamente UMA versão
ativa por nome neutro com o conteúdo 4.0:

- se não há versão ativa, cria a próxima versão (v1 em banco novo);
- se a versão ativa tem conteúdo anterior (3.0), cria uma nova versão 4.0 e
  desativa as demais, sem apagar linhas/versões históricas;
- se a versão ativa já é 4.0, é no-op (idempotente).
...
# Quatro prompts NEUTROS canônicos para novos jobs (dispatch 4.0).
PROMPT_NAMES = [
    "exam_llm1_system",
    "exam_llm1_user",
    "exam_llm2_system",
...
# Default contents do contrato neutro 4.0.
DEFAULT_CONTENTS = {
...
class Command(BaseCommand):
    ...
    def handle(self, *args: object, **options: object) -> None:
        ...
        # com o conteúdo 4.0. Se o ativo já é 4.0 → no-op; senão cria nova
        ...
        for name in PROMPT_NAMES:
            ...
            if active is not None and active.content == content:
                skipped_count += 1
                self.stdout.write(f"  Skipped (4.0 active exists): {name}")
                continue
            ...
            created_count += 1
            self.stdout.write(self.style.SUCCESS(f"  Created: {name} v{new_version} (4.0)"))
