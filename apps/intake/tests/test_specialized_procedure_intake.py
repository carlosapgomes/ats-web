"""Slice 002 (R1) — Ecoendoscopia no intake NIR sob flag independente.
...
- reenvio corrigido e correção de tipo respeitam a mesma flag;
- CPRE permanece oculta e rejeitada (chega no Slice 004);
- casos existentes não são interrompidos/apagados pela flag (web-only).
"""
...
class TestEchoendoscopyFlagValidation:
    ...
    def test_validate_exam_type_rejects_cpre(self) -> None:
        with pytest.raises(ValueError):
            validate_exam_type("cpre")
...
class TestEchoendoscopyIntake:
    ...
    def test_cpre_is_always_rejected(self, client) -> None:
        nir, _ = _nir_client(client)
