"""LLM2 Service v4 — sugestão por procedimento em uma única análise conjunta.
...
LLM2_V4_DEFAULT_SYSTEM_PROMPT = (
    ...
    "procedimentos endoscopicos digestivos. Retorne APENAS JSON valido que "
    "siga estritamente o schema_version 4.0. Escreva todos os campos narrativos "
    ...
    "support_recommendation. Produza exatamente um item em "
    "procedure_recommendations para cada procedimento reconciliado recebido: "
    "nao omita, nao duplique e nao adicione procedimento. Nao invente "
...
class Llm2V4ValidationError(RuntimeError):
    """Resposta LLM2 v4 falhou validação de schema/igualdade de conjuntos."""
...
def create_openai_llm2_v4_client() -> LlmClient:
    """Cliente LLM2 de produção: strict ``json_schema`` do contrato 4.0 (R3).

    O binding de produção do writer atual aceita as dez identidades em
    ``procedure_recommendations`` e fixa ``schema_version`` em ``"4.0"``.
    """
    return create_openai_client(
        response_schema_name="llm2_response",
        response_schema=Llm2ResponseV4.model_json_schema(),
    )


class Llm2ServiceV4:
    """Executa a análise LLM2 v4 conjunta por caso, com contrato estrito."""
        ...
        *,
        ...
        while True:
            ...
            if language_retry_used:
                ...
                raise Llm2V4ValidationError(f"LLM2 v4 output contains non-ptbr narrative terms after retry: {joined}")
...
def _render_user_prompt(
    *,
    ...
    return (
        ...
        f"Dados extraídos (JSON LLM1 v4):\n{llm1_json}\n\n"
        ...
        "Retorne JSON schema_version 4.0 com um item por procedimento reconciliado "
...
def _decode_and_validate(
    *,
    ...
    try:
        ...
        raise Llm2V4ValidationError("LLM2 v4 returned non-JSON payload") from error
    try:
        validated = Llm2ResponseV4.model_validate(decoded)
    ...
    if returned != expected:
        raise Llm2V4ProcedureSetMismatchError(
