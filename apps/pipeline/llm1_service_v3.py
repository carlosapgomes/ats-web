"""LLM1 Service v3 — extração procedure-neutral (uma chamada por caso).

Schema 3.0 (design D5/D6 / ADR-0006): valida contra ``Llm1ResponseV3``, exige
evidence spans por procedimento, aceita os quatro tipos suportados e transporta
a coleção tipada de imagem abdominal. O adapter de leitura 1.1/2.0 permanece:
artefatos antigos nunca são reescritos.
"""
...
# ── Prompts neutros canônicos schema 3.0 (fallback; seed/admin usam o mesmo) ──


LLM1_V3_DEFAULT_SYSTEM_PROMPT = (
    "Voce e um assistente clinico de triagem para Endoscopia Digestiva Alta "
    "(EDA), Colonoscopia, Ecoendoscopia e CPRE. Retorne APENAS JSON valido que "
    "siga estritamente o schema_version 3.0. Escreva todos os campos narrativos "
    "em portugues brasileiro (pt-BR). Nao use palavras em ingles nos campos "
    "narrativos. Nao inclua markdown, blocos de codigo ou chaves extras. "
...
LLM1_V3_REQUIRED_SCHEMA_INSTRUCTIONS = """
...
- schema_version: exatamente "3.0".
- language: exatamente "pt-BR".
- patient.sex: apenas "M", "F" ou "Outro".
- requested_procedures[]: 1 a 4 itens; procedure_type apenas "eda",
  "colonoscopy", "echoendoscopy" ou "cpre"; sem duplicatas.
...
LLM1_V3_DEFAULT_USER_PROMPT = (
    ...
    "Colonoscopia, Ecoendoscopia, CPRE ou combinacoes. Exigir evidencia "
    ...
    f"{LLM1_V3_REQUIRED_SCHEMA_INSTRUCTIONS}"
)


class Llm1V3ValidationError(RuntimeError):
    """Resposta LLM1 v3 falhou validação de schema/consistência."""


@dataclass
class Llm1V3Result:
    """Artefatos validados do LLM1 v3 para persistência."""
...
class Llm1ServiceV3:
    """Executa a chamada LLM1 v3 (uma por caso) com validação estrita."""
        ...
        *,
        ...
        if forbidden_terms:
            ...
            if forbidden_terms:
                ...
                raise Llm1V3ValidationError(f"LLM1 v3 output contains non-ptbr narrative terms after retry: {joined}")
...
def _render_user_prompt(
    *,
    ...
    return (
        ...
        "Retorne JSON schema_version 3.0 e preserve agency_record_number exatamente.\n"
...
def _decode_and_validate(*, raw_response: str, agency_record_number: str) -> Llm1ResponseV3:
    try:
        decoded = decode_llm_json_object(raw_response)
    except LlmJsonParseError as error:
        raise Llm1V3ValidationError("LLM1 v3 returned non-JSON payload") from error
    try:
        validated = Llm1ResponseV3.model_validate(decoded)
    except PydanticValidationError as error:
        raise Llm1V3ValidationError(f"LLM1 v3 schema validation failed: {error}") from error
    if validated.agency_record_number != agency_record_number:
        raise Llm1V3ValidationError("LLM1 v3 agency_record_number mismatch")
    return validated


def _collect_v3_forbidden_terms(*, validated: Llm1ResponseV3) -> list[str]:
    """Coleta termos em inglês nos campos narrativos do LLM1 v3."""
    texts: list[str] = [
