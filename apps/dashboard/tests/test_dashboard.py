...
# ── Slice 008: Breakdown por tipo + filtro exam_type ────────────────────
...
class TestDashboardExamTypeBreakdownAndFilter:
    ...
    def test_consolidated_counts_both_types_once(self, client) -> None:
        ...
        result = _compute_summary(period="all")
        assert result["total_today"] == 2, f"Consolidado deve somar ambos os tipos, obtido {result['total_today']}"
        assert result["accepted"] == 2, f"Aceitos consolidados = 2, obtido {result['accepted']}"
        assert result["denied"] == 0
        assert result["in_progress"] == 0
    ...
    def test_breakdown_table_headers_and_labels(self, client) -> None:
        ...
        assert "Aceitos" in content
        assert "Negados" in content
        assert "Encerrados" in content
        assert "Em Andamento" in content
        # Headers de espera por etapa
        assert "Fila Médica" in content
        assert "Fila Agendamento" in content
